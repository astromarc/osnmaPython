...
class DSM_Info():
    ...
    def __rich__(self) -> Text:
            textOsnmaStatus = Text("OSNMA:")
            textCurrentDSMID = Text ("DSM ID:")
            textCurrentBlocks = Text ("DSM Blocks:")
            textKRTOOTKey = Text ("HKRoot:")
            textKRTOOTKeyTime = Text ("HKRoot Date:")
            textKRTOOTVerificationStatus = Text ("HKRoot Auth:")
            textChainID = Text ("Chain ID:")
            textChainStatus = Text ("ChainSta:")
            textTeslaKey = Text ("TeslaKey:")
            textTeslaKeyStatus = Text ("TeslaKey Auth:")
    ...
    def colourTeslaKey(self):
        ...
        if self.__teslaKeyStatus == True:
            TeslaStatusColour.stylize("Black on Green")
        else:
    ...
    def colourRootKey(self):
        ...
        if self.__HKROOTVerificationStatus == True:
            RootStatusColour.stylize("Black on Green")
        else:
...
class svTable:
    ...
    def updateParameters(self,constellation):
        ...
        numRows = 4
        self.__tableSV = Table(title="GALILEO Space Vehicles",show_header=False, show_lines=True)
    ...
    def getCrossAuthenticationStyle(self,svId,status):
        ...
        if status[0] is not None:
            sv = sv + Text(str(status[0]))
            if status[1] == 0:
                sv.stylize("black on green")
            if status[1] == 1:
    ...
    def getProgressStyle(self,progressInt,progress):
        ...
        if progressInt == 100: progress.stylize("black on green")
        return progress
